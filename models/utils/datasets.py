...
from models.cmd.general import xyxy2xywh, xywh2xyxy, xywhn2xyxy, xyn2xy, segment2box, segments2boxes, \
    resample_segments, clean_str
from models.utils.torch_utils import torch_distributed_zero_first, hyper_parameter
...
def create_dataloader(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
    ...
    # Use torch.models.config.DataLoader() if dataset.properties will update during training else InfiniteDataLoader()
...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640, stride=32):
        ...
        for i, s in enumerate(sources):
            ...
            # # 调用TX2板载摄像头(当板载摄像头为0时需要设定此判断给予参数)
            # if url == 0:
            #     url = 'nvarguscamerasrc ! \
            ...
            #
            #     cap = cv2.VideoCapture(url, cv2.CAP_GSTREAMER)  # 开启摄像头 eval(s) if s.isnumeric() else s
            # else:
            #     cap = cv2.VideoCapture(url)
            cap = cv2.VideoCapture(url)
            assert cap.isOpened(), f'Failed to open {s}'  # 判断Camera是否开启,否则引发异常
            # 默认加载Camera最大分辨率
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        if self.augment:
            ...
            if not mosaic:
                img, labels = random_perspective(img, labels,
                                                 degrees=hyperparameter.degrees,
                                                 translate=hyp['translate'],
                                                 scale=hyp['scale'],
                                                 shear=hyperparameter.shear,
                                                 perspective=hyperparameter.perspective)
        ...
        if self.augment:
            # flip up-down
            if random.random() < hyperparameter.flipud:
                img = np.flipud(img)
                if nL:
                    labels[:, 2] = 1 - labels[:, 2]
...
def load_mosaic(self, index):
    ...
    img4, labels4, segments4 = copy_paste(img4, labels4, segments4, probability=hyperparameter.copy_paste)
...
def load_mosaic9(self, index):
    ...
    img9, labels9, segments9 = copy_paste(img9, labels9, segments9, probability=hyperparameter.copy_paste)
...
def extract_boxes(path='../coco/'):  # from models.datasets import *; extract_boxes('../coco128')
...
def autosplit(path='../coco', weights=(0.9, 0.1, 0.0), annotated_only=False):
    ...
    Usage: from models.datasets import *; autosplit('../coco')
    Arguments
